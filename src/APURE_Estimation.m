...
% unbiased estimation risk estimate of Equations (28) and (36) of
% Pascal & Vaiter (2024).
%
%
%
...
function [X, lambda, oracle] = APURE_Estimation(Y,Psi_Y,M,opts)
    ...
    % robustfied Finite Difference Monte Carlo APURE unbiased estimation
    % risk estimate w.r.t. lambda:
    %
    %      lambda_APURE = argmin_lambda 1/N sum_n APURE_E[zeta_n](Y; lambda | alpha)
    %
    ...
    %            - RISK: minimizing true estimation risk (if ground truth is provided in M.X)
    ...
    %            - RISK: Quadratic estimation risk (if ground truth is provided in M.X)
    ...
    C            = sum(Psi_Y.^(-2) .* Y.^2) - M.alpha*sum(Psi_Y.^(-2) .* Y) ;
    ...
    for n = 1 : opts.N
        ...
        for ell = 1 : opts.L
            ...
            % Compute the Finite Difference Monte Carlo APURE unbiased estimation risk estimate
            APURE_MC(n,ell)       =  norm(X_lambda , 'fro')^2 - 2 * sum(Y .* Psi_Y.^(-1) .* X_lambda) + 2 * M.alpha * sum(Y .* Psi_Y.^(-1) .* dX_lambda .* dY) + C ;

            % Compute true estimation risk if ground truth available
            if n == 1
                if isfield(M,'X')
                    RISK(ell)     = norm(M.X - X_lambda,'fro')^2 ;
                end
            end
    ...
    if isfield(M,'X')
        % Best regularization parameter in terms of estimation risk
    ...
    if sum(isnan(Z))      == length(Z)
        ...
        if isfield(opts,'flag')
            disp(['Computing the estimate with optimal parameter minimizing ',opts.flag,'...'])
        end
