% Display the estimates of the COVID-19 reproduction number either 
...
function display_Covid_Manual(R, FontSize)

    % Inputs:  - R: estimated piecewise linear reproduction numbers
    ...
    if ~isfield(R,'Dates');    R.Dates = 1:length(R.ML);    end
    ...
    if isfield(R,'GT')
        q            = plot(R.Dates,R.GT,'linewidth',3,'color',royal) ;
        Q            = [Q, q] ;
    ...
    if isfield(R,'PKL')
        q            = plot(R.Dates,R.PKL,'linewidth',3,'color',brown) ;
        Q            = [Q, q] ;
        L{iEst}      = '$\widehat{\mathrm{R}}_t(\mathrm{Y};\lambda)$' ;
        M            = max(M, max(R.PKL)) ;
        iEst         = iEst + 1 ;
    end
    ...
    elseif length(Q) == 2
        l3.Position  =  [0.7870 0.6894 0.1180 0.1961] ;
    end
    l3.FontSize      = FontSize ;
    l3.Color         = 'None' ;
    title('Estimates of the reproduction numbers with no or manual tuning','Interpreter','Latex')
